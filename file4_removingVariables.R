...
datGdImpTrain %<>%
  ...
  # Important variables selected from previous file3_ grid search. They are more important that ID
  select("bedroom_abv_gr",  "garage_type",     "wood_deck_sf",    "bsmt_unf_sf",    
"neighborhood",    "open_porch_sf",   "lot_frontage",    "mas_vnr_area",   
"fireplaces",      "garage_finish",   "year_remod_add",  "tot_rms_abv_grd",
"lot_area",        "alley",           "garage_yr_blt",   "full_bath",      
"bsmt_fin_sf1",    "kitchen_qual",    "x2nd_flr_sf",     "bsmt_qual",      
"year_built",      "x1st_flr_sf",     "exter_qual",      "garage_area",    
"total_bsmt_sf",   "garage_cars",     "gr_liv_area",     "overall_qual", "sale_price") 


#------------MODEL   ---------- 
#---Apply an algorithm - reandomForest -> ranger
#--- Apply an algorithm (random forest, theorem of no free lunch, but random forest it's a great start solution, invariant to variables transforms)

#--- Tune mode with gridsearch --- START INTERATION 3
tic()

#standardize name for future improvements
mytrain <- datGdImpTrain

#--- Greed to search
mytrees <- c(250, 500, 750, 1000)
mymtry <- c(6,7,8,9,10,11,12)
mygrid <- expand.grid(mytrees = mytrees,
                      mymtry = mymtry)

#--- To store results
res_val <- data.frame(trees = 0, mtry = 0, accu_val = 0)
for (i in 1:nrow(mygrid)){
  
  mymodel <- ranger(
    sale_price ~ .,
    data = mytrain,
    num.trees = mygrid$mytrees[i],
    mtry = mygrid$mymtry[i],
    # to get variable importance for a classification   model
    importance = "impurity" 
  )
  ...
  res_val[i,] <- rbind(c(mygrid$mytrees[i],
                         mygrid$mymtry[i],
                         accu_val))
  print(res_val)
...
best_param <- res_val %>%
  filter(accu_val == max(res_val$accu_val))

#--- END INTERATION 3 - gridsearch

#--- Get the model 
mymodel <- ranger(
  sale_price ~ .,
  data = mytrain,
  num.trees = best_param$trees,
...
#-- Save submission
fwrite(sub_df,
       paste0("./submissions/file4_ranger_vars_", nrow(varImp),
...
## Random forest - Grid search submission score on platform: 0.15003 error <- worse
## ## Random forest - Grid search - Most important variables submission score on platform: 0.14911 error <- worse