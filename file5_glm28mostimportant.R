...
model1 <- glm(sale_price ~ overall_qual + gr_liv_area + garage_cars + total_bsmt_sf +
                ...
                fireplaces + tot_rms_abv_grd + open_porch_sf + garage_type +
                neighborhood + bsmt_unf_sf + wood_deck_sf + bedroom_abv_gr +
                half_bath + sale_condition + overall_cond + lot_frontage +
                mas_vnr_area + garage_finish + alley + garage_yr_blt + bsmt_qual,
              data = datGdImpTrain)
...
#-- Save submission
fwrite(sub_df, 
       paste0("./submissions/file5_glm_vars_", nrow(varImp),
              "_acc_", round(accu_val,4), ".csv"), nThread = 3
)
